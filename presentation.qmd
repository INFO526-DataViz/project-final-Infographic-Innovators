...
::: nonincremental
Trends in aircraft crashes, focusing on fatalities and injury types.
Highlights notable historical crashes and examines fatalities `fatal_injury_count`, serious injuries `serious_injury_count`, and minor injuries `minor_injury_count`.
...
## HeatMap {.smaller}
::: nonincremental
-   Animated visualization of crashes across the U.S. over time.
-   Utilizes latitude and longitude coordinates to illustrate regional frequency and patterns.
...
::: nonincremental
-   Overview of common causes, utilizing the probable_cause column.
-   Employs text summarization to categorize causes.
...
::: nonincremental
-   Immense amount of Data to work from.
-   The obtained dataset was partially skewed which made it harder to normalise certain metrics for visualisation.
-   Performing Data Wrangling on the dataset was difficult as it was of size `89,134 rows and 20 columns`.
-  But, because the Dataset was vast, we could also create multiple visualisations which we could present for this project.
:::
...
-   Diving in Deeper into flight crashes and visualising flight paths and plotting real-time status of flights.
:::
