...
#### Waffle chart
```{r loading probablcause data, include=FALSE}
probable_cause_data <- read_csv(here("data", "new_flights_PC.csv"))
probable_cause_data <- subset(probable_cause_data, probable_cause_data$HighestInjuryLevel!="")
```

```{r datawragling,include=FALSE}
count <- probable_cause_data |>
...
```{r function-waffle}
waffle_plot <- function(number, colour, symbol, symbol_size=8) {
  p <- expand.grid(x = 0:9,
                   y = 0:9) |>
    rowwise() |>
    ...
    coord_equal() +
    theme_void() +
    labs(title="Waffle chart showing different causes for crashes",
         color = '')
  return(p)
}
```
...
waffle_plot(number = count_waffle$number,
            colour = count_waffle$Probable_Cause,
...
```{r distribution}
ggplot(subset(probable_cause_data, probable_cause_data$Probable_Cause=="pilot's failure")) +
  geom_density(aes(x=year(EventDate), fill=HighestInjuryLevel))+
