...
radar_plot_weather <- ggplot(flights_ntsb_radar, aes(x = event_month, y = total_crashes, 
                               group = weather_condition, color = weather_condition, fill = weather_condition)) +
...
ui <- fluidPage(
  ...
  sidebarLayout(
    ...
    mainPanel(
      ...
      # Output: Histogram ----
      plotOutput(outputId = "radar_plot_weather")
...
server <- function(input, output) {
  ...
  output$radar_plot_weather <- renderPlot({
...
runApp("~/Desktop/INFO-526/project-final-Infographic-Innovators")